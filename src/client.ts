...
export class HitobitoClient {
  ...
  private buildQuery(options?: QueryOptions<Record<string, unknown>, string>): string {
    if (!options) return "";
  ...
  async getRole(id: number): Promise<Role> {
    ...
    if (!parsed.success) {
      throw new ValidationError(`Invalid role response: ${parsed.error.message}`);
    }
  ...
  async getInvoice(id: number): Promise<Invoice> {
    ...
    if (!parsed.success) {
      throw new ValidationError(`Invalid invoice response: ${parsed.error.message}`);
    }
  ...
  async getMailingList(id: number): Promise<MailingList> {
    ...
    if (!parsed.success) {
      throw new ValidationError(`Invalid mailing list response: ${parsed.error.message}`);
    }
  ...
  async getEventKind(id: number): Promise<EventKind> {
    ...
    if (!parsed.success) {
      throw new ValidationError(`Invalid event kind response: ${parsed.error.message}`);
    }
  ...
  async getEventKindCategory(id: number): Promise<EventKindCategory> {
    ...
    if (!parsed.success) {
      throw new ValidationError(`Invalid event kind category response: ${parsed.error.message}`);
    }

    return parsed.data;
  }

  async getPeople(options?: QueryOptions<PeopleFilter, PeopleSortKey>): Promise<Person[]> {
    const qs = this.buildQuery(options as QueryOptions<Record<string, unknown>, string>);
  ...
  async getGroups(options?: QueryOptions<GroupsFilter, GroupsSortKey>): Promise<Group[]> {
    const qs = this.buildQuery(options as QueryOptions<Record<string, unknown>, string>);
  ...
  async getEvents(options?: QueryOptions<EventsFilter, EventsSortKey>): Promise<Event[]> {
    const qs = this.buildQuery(options as QueryOptions<Record<string, unknown>, string>);
  ...
  async getRoles(options?: QueryOptions<RolesFilter, RolesSortKey>): Promise<Role[]> {
    const qs = this.buildQuery(options as QueryOptions<Record<string, unknown>, string>);
  ...
  async getInvoices(options?: QueryOptions<InvoicesFilter, InvoicesSortKey>): Promise<Invoice[]> {
    const qs = this.buildQuery(options as QueryOptions<Record<string, unknown>, string>);
    const data = await this.request<unknown>(`/invoices${qs}`);
    const parsed = InvoicesResponseSchema.safeParse(data);

    if (!parsed.success) {
      throw new ValidationError(`Invalid invoices response: ${parsed.error.message}`);
    }

    return parsed.data;
  }

  async getMailingLists(options?: QueryOptions<MailingListsFilter, MailingListsSortKey>): Promise<MailingList[]> {
    const qs = this.buildQuery(options as QueryOptions<Record<string, unknown>, string>);
    const data = await this.request<unknown>(`/mailing_lists${qs}`);
    const parsed = MailingListsResponseSchema.safeParse(data);

    if (!parsed.success) {
      throw new ValidationError(`Invalid mailing lists response: ${parsed.error.message}`);
    }
  ...
  async getEventKinds(options?: QueryOptions): Promise<EventKind[]> {
    ...
    if (!parsed.success) {
      throw new ValidationError(`Invalid event kinds response: ${parsed.error.message}`);
    }

    return parsed.data;
  }

  async getEventKindCategories(options?: QueryOptions): Promise<EventKindCategory[]> {
    const qs = this.buildQuery(options as QueryOptions<Record<string, unknown>, string>);
    ...
    if (!parsed.success) {
      throw new ValidationError(`Invalid event kind categories response: ${parsed.error.message}`);
    }
